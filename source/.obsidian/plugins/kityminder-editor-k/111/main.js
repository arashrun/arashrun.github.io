...
var KityMinderNoteModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName("Markdown \u5907\u6CE8").setDesc("\u76F4\u63A5\u8F93\u5165 Markdown\uFF0C\u53F3\u4FA7/\u4E0B\u65B9\u4F1A\u5B9E\u65F6\u9884\u89C8");
    const editorContainer = contentEl.createDiv({ cls: "kityminder-modal-editor" });
    editorContainer.style.display = "flex";
    editorContainer.style.gap = "12px";
    editorContainer.style.marginBottom = "16px";
    ...
    this.previewEl = editorContainer.createDiv({ cls: "kityminder-note-preview" });
    ...
    renderResources();
    new import_obsidian.Setting(contentEl).setName("\u6DFB\u52A0 Obsidian \u7B14\u8BB0\u94FE\u63A5").addButton(
      (btn) => btn.setButtonText("\u6D4F\u89C8 Vault").onClick(async () => {
        const files = this.app.vault.getMarkdownFiles();
        const picker = new import_obsidian.Setting(contentEl).setName("\u9009\u62E9\u7B14\u8BB0").addDropdown((dropdown) => {
          var _a;
          files.sort((a, b) => a.path.localeCompare(b.path));
          files.forEach((f) => dropdown.addOption(f.path, f.path));
          let selected2 = ((_a = files[0]) == null ? void 0 : _a.path) || "";
          dropdown.onChange((v) => selected2 = v);
          dropdown.setValue(selected2);
          return dropdown;
        }).addButton(
          (confirmBtn) => confirmBtn.setButtonText("\u786E\u8BA4").onClick(() => {
            const file = files.find((f) => f.path === selected);
            if (file) {
              this.resources.push({ name: file.basename, path: file.path });
              renderResources();
            }
            picker.settingEl.remove();
          })
        );
      })
    ).addText((text) => {
      text.setPlaceholder("\u6216\u624B\u52A8\u8F93\u5165 WikiLink \u5982 [[\u7B14\u8BB0\u540D]]");
      let manualValue = "";
      text.onChange((v) => manualValue = v);
      return text;
    }).addButton(
      (btn) => btn.setButtonText("\u6DFB\u52A0").onClick(() => {
        var _a;
        const input = ((_a = btn.buttonEl.previousElementSibling) == null ? void 0 : _a.value) || "";
        const match = input.match(/\[\[(.*?)\]\]/);
        const name = match ? match[1] : input;
        if (name.trim()) {
          const file = this.app.vault.getFiles().find((f) => f.basename === name.trim() || f.path === name.trim());
          this.resources.push({
            name: name.trim(),
            path: file ? file.path : name.trim()
          });
          renderResources();
        }
      })
