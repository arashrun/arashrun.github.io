...
1. **KMS 到底管什么？**
KMS 的核心任务是管理显卡的显示管道。它主要处理以下三件事：
分辨率 (Resolution)： 设置屏幕的长宽像素。
刷新率 (Refresh Rate)： 决定屏幕每秒更新多少次。
色深 (Color Depth)： 管理色彩显示的位数。

2. **为什么需要 KMS？（痛点解决）** 
在 KMS 普及之前，显卡驱动主要运行在“用户级”（User Space）。这带来了几个大麻烦：
开机黑屏/闪烁： 内核启动时用的是简单的 VGA 模式，等 X11 启动后又强行切换驱动，导致屏幕猛地黑一下。
VT 切换缓慢： 当你从桌面按 Ctrl+Alt+F1 切换到纯文本控制台时，系统要重新初始化显卡，反应非常慢。
死机即“失明”： 如果 X Server 崩溃了，因为控制权在它手里，你的整个屏幕往往会直接卡死或变花，连内核报错（Kernel Panic）的信息都看不见。

3. **KMS 与 DRM 的关系** 
你常会听到 DRM/KMS 这个组合词。它们的关系就像是一台电脑的“大脑”和“眼睛”：
DRM (Direct Rendering Manager)： 负责管“算力”和“显存”，处理 3D 渲染和计算。
KMS： 负责管“输出”，决定渲染好的画面怎么投射到你的显示器上。
如今，KMS 已经成为了 DRM 子系统的一部分。


## Term

- kms:
- drm:DRM (Direct Rendering Manager) 是 Linux 内核中负责与 GPU 通信的子系统。