...
:echo expand('%:t')      " 只显示文件名
:cd %:p:h                " 切换工作目录到当前文件所在的文件夹
```

| **修饰符** | **含义**                | **示例 (假设文件为 src/main.lua)**       |
| ------- | --------------------- | --------------------------------- |
| `:p`    | **P**ath：转换为绝对全路径     | `/home/user/project/src/main.lua` |
| `:t`    | **T**ail：提取文件名（尾部）    | `main.lua`                        |
| `:r`    | **R**oot：去掉最后一次出现的扩展名 | `src/main`                        |
| `:e`    | **E**xtension：提取扩展名   | `lua`                             |
| `:h`    | **H**ead：提取目录部分（头部）   | `src`                             |
| `:.`    | 转换为相对于当前工作目录的路径       | `src/main.lua`                    |
| `:~`    | 转换为相对于家目录 (`~`) 的路径   | `~/project/src/main.lua`          |
